...
export const LessonNodeCard: React.FC<LessonNodeCardProps> = ({ node, isLastInSection, onSelect }) => {
    ...
    const content = (
        <div className="relative pl-12 sm:pl-16 w-full max-w-2xl mb-4">
            ...
            <div
                className={`flex w-full min-w-0 items-center justify-between overflow-hidden rounded-xl border-2 p-4 shadow-sm transition-transform hover:-translate-y-0.5 ${safeHref ? 'cursor-pointer' : ''} ${cardSurfaceClass}`}
            >
                ...
                <div className="flex min-w-0 items-center gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                        <h3 className={`mb-0.5 flex min-w-0 flex-wrap items-center gap-2 text-[15px] font-bold ${isInProgress ? 'text-gray-900' : isDone ? 'text-emerald-950' : isFailed ? 'text-red-900' : style.title}`}>
                            <span className="min-w-0 break-words [overflow-wrap:anywhere]">{node.title}</span>
                            {isInProgress ? (
                        ...
                        <div className="mt-1 min-w-0">
                            {isInProgress ? (
                                ...
                                <span className={`block break-words [overflow-wrap:anywhere] ${style.textInfo}`}>Start now</span>
                            ) : node.type === 'test' ? (

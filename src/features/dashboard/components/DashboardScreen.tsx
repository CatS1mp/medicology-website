...
import { getCourseContentIds, getMainFinalizedAttempts } from '@/shared/utils/learning-progress';
...
export const DashboardScreen: React.FC = () => {
    ...
    const setLearningProgressData = useUserStore(state => state.setLearningProgressData);
    const initialShowIntroLoading = !hasSeenDashboardLoading;
    const shouldShowIntroLoading = React.useRef(initialShowIntroLoading);
    ...
    const [showBotOverlay] = React.useState(initialShowIntroLoading);
        ...
        async function run() {
            ...
            try {
                const dataPromise = Promise.all([
                    ...
                    getCourses().catch(() => []),
                    getMyAttempts().catch(() => []),
                ]) as Promise<[CourseProgressResponse[], CourseResponse[], AttemptSummaryResponse[]]>;
                
                if (shouldShowIntroLoading.current) {
                    await Promise.all([dataPromise, new Promise(resolve => setTimeout(resolve, 2000))]);
                }

                const [progress, courses, attempts] = await dataPromise;
                if (cancelled) return;
                setLearningProgressData(progress, attempts);
                ...
                const finalizedAttempts = getMainFinalizedAttempts(attempts);
                const sortedCourses = courses.slice().sort((a, b) => a.orderIndex - b.orderIndex);
                const courseOrderById = new Map(sortedCourses.map((course, index) => [course.id, index]));
                ...
                sortedCourses.forEach((course) => {
                    ...
                    const completed = contentIds.filter((id: string) => passedContentIds.has(id)).length;
                ...
                const resolvedProgressItems = progress
                        ...
                        .sort((a, b) => {
                            ...
                            return (courseOrderById.get(a.id) ?? Number.MAX_SAFE_INTEGER) - (courseOrderById.get(b.id) ?? Number.MAX_SAFE_INTEGER);
                        })
                        .slice(0, 5);
    ...
    }, [setHasSeenDashboardLoading, setLearningProgressData]);
    ...
    React.useEffect(() => {
        ...
        async function loadLessonActivity() {
            ...
            try {
                ...
                getMainFinalizedAttempts(attempts)

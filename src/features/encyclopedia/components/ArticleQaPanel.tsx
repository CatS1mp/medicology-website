    ...
    return (
        ...
        >
            ...
            {message.role === 'assistant' && message.outOfScope && (
                <p className="mt-1 text-[10px] font-medium text-amber-600">Cau hoi nam ngoai pham vi bai viet.</p>
            )}
    ...
    useEffect(() => {
        ...
        async function loadSuggested() {
            try {
                ...
                if (!cancelled) {
                    const fromToc = [
                        'Tom tat noi dung chinh cua bai viet',
                        ...tableOfContents.slice(0, 3).map((item) => `Giai thich muc: ${item.label}`),
                    ];
    ...
    const submitQuestion = useCallback(
        async (rawQuestion: string) => {
            ...
            try {
                ...
                const message = e instanceof Error ? e.message : 'Khong gui duoc cau hoi.';
                setError(message);
        ...
        <>
            ...
            {!isLoggedIn ? (
                ...
                <>
                    ...
                    >
                        {!messages.length && (
                            <p className="text-xs text-gray-400">Dat cau hoi ve noi dung bai viet.</p>
                        )}
                        {messages.map((message) => (
                            <ChatMessageBubble key={message.id} message={message} />
                        ))}
                        {loading && <p className="text-xs text-gray-400">Dang tra loi...</p>}
                    </div>
    ...
    if (variant === 'inline') {
        return (
            <div className="mb-8 rounded-2xl border border-[#8ed0f8] bg-white p-5 shadow-[0_6px_20px_rgba(15,23,42,0.04)] lg:hidden">
                ...
                >
                    <span className="text-sm font-bold text-[#1CA1F2]">Hoi AI ve bai viet</span>
                    <span className="text-xs text-gray-400">{mobileOpen ? 'Thu gon' : 'Mo rong'}</span>
                </button>
                {mobileOpen && (

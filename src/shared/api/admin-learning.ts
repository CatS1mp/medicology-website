...
export async function adminCreateCourse(body: {
    ...
    return requestApi<CourseResponse>(`${API}/courses`, {
        method: 'POST',
...
export async function adminListSections(courseId: string): Promise<SectionResponse[]> {
    return requestApi<SectionResponse[]>(`${API}/courses/${encodeURIComponent(courseId)}/sections`, {
        method: 'GET',
        headers: buildHeaders(),
    });
}
...
export async function adminListLessons(sectionId: string): Promise<LessonResponse[]> {
    return requestApi<LessonResponse[]>(`${API}/sections/${encodeURIComponent(sectionId)}/lessons`, {
        method: 'GET',
        headers: buildHeaders(),
    });
}
...
export async function adminCreateLesson(body: {
    ...
    return requestApi<LessonResponse>(`${API}/lessons`, {
        method: 'POST',
        headers: buildHeaders(),
        body: JSON.stringify(body),
    });
...
export async function adminUpdateLesson(lessonId: string, body: Partial<LessonResponse>): Promise<LessonResponse> {
    return requestApi<LessonResponse>(`${API}/lessons/${encodeURIComponent(lessonId)}`, {
        method: 'PUT',
        headers: buildHeaders(),
        body: JSON.stringify(body),
    });
...
export async function adminPatchLessonStatus(lessonId: string, isActive: boolean): Promise<LessonResponse> {
    return requestApi<LessonResponse>(`${API}/lessons/${encodeURIComponent(lessonId)}/status`, {
        method: 'PATCH',
        headers: buildHeaders(),
